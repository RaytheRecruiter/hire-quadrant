...
-- NOTE: job_applications.id is TEXT, so application_id must also be text.
...
-- Heal partial runs that created the column as uuid. Policies that
-- reference application_id block ALTER COLUMN, so drop them first.
