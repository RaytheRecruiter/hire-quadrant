    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full h-full sm:h-auto sm:max-h-[90vh] sm:rounded-3xl sm:max-w-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <div>
            <h2 className="text-lg font-bold text-secondary-900">{candidate?.name || application.user_name || 'Applicant'}</h2>
            <p className="text-sm text-gray-500">{candidate?.email || application.user_email}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          {/* Star rating */}
          <div>
            <label className="block text-sm font-semibold text-secondary-800 mb-2">Candidate Rating</label>
            <div className="flex items-center gap-1">
              ...
              <span className="ml-3 text-sm text-gray-500">{rating > 0 ? `${rating} / 5` : 'No rating'}</span>
            </div>
          </div>
          ...
          <div>
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <Bookmark className={`h-5 w-5 ${shortlisted ? 'fill-primary-500 text-primary-500' : 'text-gray-400'}`} />
              <span className="font-medium text-secondary-800">Shortlist this candidate</span>
            </label>
          </div>
          ...
          {/* Notes */}
          <div>
            <label className="block text-sm font-semibold text-secondary-800 mb-2">Private Notes</label>
            <textarea
              value={notes}
          ...
          <div className="border border-primary-200 rounded-2xl p-4 bg-primary-50/30">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-secondary-800">AI Candidate Screening</span>
              </div>
              <button
            ...
            {aiResult && (
              <div className="mt-4 space-y-3 text-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-gray-500">Fit score /100</div>
                </div>
                <p className="text-gray-800">{aiResult.summary}</p>
          ...
          {application.screening_answers && application.screening_answers.length > 0 && (
            <div className="bg-gray-50 rounded-xl p-4">
              <p className="text-sm font-semibold text-secondary-800 mb-3">Screening Answers</p>
              <ul className="space-y-2 text-sm">
                {application.screening_answers.map((a: any, i: number) => (
                  <li key={i}>
                    <span className="text-gray-500">Q{i + 1}:</span>{' '}
                    <span className="text-gray-800 font-medium">{a.answer || '—'}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="border-t border-gray-200 px-6 py-4 flex items-center justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-gray-600 font-medium hover:bg-gray-100">
            Cancel
          </button>
          <button
            onClick={handleSave}
