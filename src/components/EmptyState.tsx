...
const EmptyState: React.FC<Props> = ({ illustration, icon, title, description, primaryAction, secondaryAction }) => {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-soft p-10 md:p-14 text-center animate-fade-in">
      <div className="mx-auto w-40 h-40 mb-6">
        {illustration || (
          <div className="w-full h-full bg-primary-50 rounded-3xl flex items-center justify-center text-primary-400">
            {icon}
          </div>
        )}
      </div>
      <h3 className="font-display text-2xl font-bold text-secondary-900 mb-2 text-balance">{title}</h3>
      <p className="text-gray-500 mb-7 max-w-md mx-auto text-balance leading-relaxed">{description}</p>
